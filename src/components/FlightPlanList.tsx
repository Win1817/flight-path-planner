import { Plane, Clock, Layers, MapPin } from 'lucide-react';
...
export function FlightPlanList({ plans, selectedPlanId, onSelectPlan }: FlightPlanListProps) {
  if (plans.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3">
      {plans.map((plan) => {
        ...
        const isSelected = selectedPlanId === plan.operation_plan_id;

        return (
          <div
            key={plan.operation_plan_id}
            onClick={() => onSelectPlan(plan.operation_plan_id)}
            className={cn('flight-card', isSelected && 'active')}
          >
