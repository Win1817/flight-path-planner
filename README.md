...
| **Input formats** | OPS: both ED-269 (plan fields at the top level) and ED-318 (plan fields nested under `operationPlan`) schemas; a single plan object, an array, or an object with a `plans`, `operations` or `flight_plans` array; both camelCase and snake_case field names. The two OPS schemas aren't treated as one status vocabulary: each record keeps a schema tag (an "ED-318" badge on its card), and ED-318's approval/take-off-clearance/conflict data (which ED-269 has no equivalent of) is shown in its own "ED-318 APPROVAL" section rather than folded into ED-269's State/Closure Reason pills. AoR: two schemas (a "responsibility area" schema and an airspace-zone schema with Circle/Polygon/MultiPolygon projections); entries that cannot be parsed are skipped and counted in a warning. |
...
- **Viewport map rendering** – above 1,500 shapes the map only receives what is in view (NetTopologySuite STRtree), sent as chunked, coordinate-rounded GeoJSON; zoom in to see more. Shapes are also simplified to what the visible span can resolve (about one screen pixel) and capped at 300,000 vertices per update, because a file reaches hundreds of MB through vertex-heavy shapes as easily as through record count (a 147 MB file of 7,000 zones x 1,000 vertices produced a 52 MB first render before this; it is now 6.9 MB). Diagnostics are appended to `import.log` next to the executable.
